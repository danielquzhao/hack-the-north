...
The available controls are implemented in the app; the AI selects and arranges them. You can request a presentation controller, gamepad buttons, a thumbstick, tilt pointer control, or separate swipe, pinch, and rotation pads. The phone reads motion only while a tilt control is visible; tap its card to recenter. The Mac editor gives each gesture direction its own keyboard mapping. Gestures are recognized only within their pads, so they do not interfere with buttons or the thumbstick.
