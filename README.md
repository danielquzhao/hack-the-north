...
The available controls are implemented in the app; the AI selects and arranges them. You can request a presentation controller, gamepad buttons, a thumbstick, tilt pointer control, drag pad, or separate discrete pinch and rotation pads. A drag pad holds a configurable left, right, or middle Mac mouse button while you move one finger and scrolls continuously as you pinch with two fingers. For Google Earth, use a left drag and pinch to zoom; for Blender viewport orbit, use a middle drag and pinch to zoom. Set the Mac pointer over the target viewport before using the pad. The Mac editor lets you change the drag button, modifiers, speed, and zoom speed. The phone reads motion only while a tilt control is visible; tap its card to recenter. Rebuild both Mac and iPhone apps before pairing because the controller schema and pairing protocol changed.
